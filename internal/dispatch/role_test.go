...
// A `hitl` card is the owner's own hands, so Start must refuse it outright
// rather than resolve some role for it.
//
// Measured live before this gate: the board's role picker opened on
...
func TestStartRefusesHumanOnlyCards(t *testing.T) {
	...
	for name, req := range map[string]Request{
		...
		// gate placed after the explicit-role branch would be bypassed
...
// configured default, which is what roles.Load's doc promises and what any
// board without an agents directory has always done.
func TestStartStillDispatchesWhenNoCharterMatches(t *testing.T) {
	...
	_, err := dispatcher.Start(context.Background(), Request{Spec: card, Kind: unknownHarness})
	if errors.Is(err, roles.ErrHumanOnly) {
	...
	role, roleErr := dispatcher.resolveRole(Request{Spec: card}, cfg.Column(card.Status))
