...
// ago, on a repository with a 30m timer.
func openPR(now time.Time, quiet time.Duration) Input {
	return Input{
		Policy:   line(),
		Labels:   []string{"hvb:source:pr", "hvb:pr:64", LabelStateOpen, "hvb:check:green", activity(now.Add(-quiet))},
...
func TestReasonsRoundDurationsToTheSecond(t *testing.T) {
	...
	for _, quiet := range []time.Duration{31 * time.Minute, 29*time.Minute + 59*time.Second} {
		got := Resolve(openPR(now, quiet))
		if strings.Contains(got.Reason, ".") {
			t.Errorf("quieto por %s: motivo %q tem fração de segundo", quiet, got.Reason)
		}
	}
}
